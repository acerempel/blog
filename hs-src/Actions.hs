...
templateRule :: (FilePath -> Route)
             -> (Route -> Action (Html ()))
             -> ReaderT FilePath Rules ()
templateRule routeBuilder template =
   urlRule routeBuilder $ \route buildDir -> do
      ...
      liftIO $
        IO.withFile targetFile IO.WriteMode $
          \targetHandle -> hPutBuilder targetHandle htmlBytes

urlRule :: (FilePath -> Route)
        -> (Route -> FilePath -> Action ())
        -> ReaderT FilePath Rules ()
urlRule routeBuilder action = ReaderT $ \buildDir -> do
   let pattern =
         buildDir </> Routes.targetFile (routeBuilder "*")
   pattern %> \targetFile -> do
      let (Just [filename]) =
            filePattern pattern targetFile
          route = routeBuilder filename
      putLoud $ "Calling rule for url "
             <> show (Routes.url route) <> ", file " <> targetFile
